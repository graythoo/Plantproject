...
if(isset($mydata["username"]) && isset($mydata["password"])){
    if(($mydata["username"]) != "" && $mydata["password"] != ""){
        ...
        $link = create_connect();
        //找出相同帳號的那一筆資料
        $sql="SELECT Username,Password,UserState FROM user WHERE Username='$p_username'";
        ...
        if(mysqli_num_rows($result) == 1){
            ...
            if(password_verify($p_password,$password_hash)){
                ...
                $sql = "UPDATE user SET UID01 = '$uid01', UID02 = '$uid02' WHERE Username = '$p_username'";
                execute_sql($link,"plantdb",$sql);

                //撈取除了密碼以外的資訊
                $sql="SELECT ID,Username,UserState,UID01,UID02 FROM user WHERE Username = '$p_username'";
                $result = execute_sql($link,"plantdb",$sql);
                $row = mysqli_fetch_assoc($result);
                $userData = array();
                $userData[] = $row;//轉成陣列
